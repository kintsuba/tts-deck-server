...
export const mergeCardDescriptorSchema = z.object({
  id: z.string().uuid({ message: 'Each card id must be a valid UUID' }),
  imageUri: z.string().url({ message: 'imageUri must be a valid URL' }),
});

export const mergeRequestSchema = z
  .array(mergeCardDescriptorSchema)
  .min(1, 'At least one card image must be provided')
  .max(MAX_CARDS, `A maximum of ${MAX_CARDS} images are supported for TTS import`);
