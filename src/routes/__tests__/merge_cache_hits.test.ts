...
import { getPutCommands, mockCachedImage, mockCachedImageMissing, resetS3Mock } from '../../test/utils/s3Mock';
...
const createTestImage = async (seed: number) => {
  return sharp({
    create: {
      ...
      height: 170,
      channels: 4,
      background: { r: seed % 255, g: (seed * 5) % 255, b: (seed * 11) % 255, alpha: 1 },
    },
  })
    .png()
    .toBuffer();
...
test('POST /merge reuses cached entries without refetching', async () => {
  ...
  for (let index = 0; index < 70; index += 1) {
    ...
    if (index % 2 === 0) {
      ...
      mockCachedImage(`cache/${id}`, buffer, 'image/png', { bucket: TEST_BUCKET });
      cachedIds.add(id);
    } else {
      const buffer = await createTestImage(index);
  ...
  try {
    ...
    const metadata = JSON.parse(Buffer.from(metadataHeader, 'base64url').toString('utf8')) as Record<string, unknown>;
