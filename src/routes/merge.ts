...
export const registerMergeRoute = (app: Hono) => {
  app.post('/merge', async (c) => {
    ...
    const headerBytes = contentLength ? Number.parseInt(contentLength, 10) : Number.NaN;
    ...
    try {
      ...
      if (error instanceof ImageProvisionError) {
        const status = (error.status && error.status >= 400 && error.status <= 599
          ? error.status
        ...
        const response = c.json(
          {
            ...
            source: error.code?.includes('cache') ? 'image_cache' : 'image_fetch',
            code: error.code,
          },
          status,
        );
