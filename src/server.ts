...
export const getApp = () => {
  ...
  app.use('*', async (c, next) => {
    ...
    try {
      ...
      const metrics = (c.get('mergeMetrics') as Record<string, unknown> | undefined) ?? {};
      const status = (c.res?.status ?? c.get('responseStatus')) as number | undefined;
  ...
  app.onError((err, c) => {
    if (err instanceof HTTPException) {
      ...
      const status = (candidate >= 100 && candidate <= 599
        ? candidate
