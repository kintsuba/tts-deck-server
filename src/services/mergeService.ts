...
export const executeMerge = async (request: MergeRequest): Promise<MergeResult> => {
  ...
  const images = await mapConcurrently(request.cards, FETCH_CONCURRENCY, async (card) => {
    ...
    try {
      ...
      if (error instanceof ImageProvisionError && error.code === 'merge.image_fetch_failed') {
        downloadFailures += 1;
  ...
  const cachedIds = images.filter((image) => image.wasCached).map((image) => image.id);
  const downloadedIds = images.filter((image) => !image.wasCached).map((image) => image.id);
  ...
  const totalFetchDuration = fetchDurations.reduce((acc, value) => acc + value, 0);
